...
**Takeaway: leverage the `tidyverse` for your daily data tasks, but ensure you understand the fundamentals of base R!**
...
**Remember:** often the choice of **data structure** determines the **data type(s)** that can be stored within it... so let's reveiw data structures!
...
* the data type of their content:
  * homogeneous or all contents must be of the same data type: vectors and matrices
  * heterogeneous or contents can be of different data types: lists and data frames
...
<!-- why dim() is NULL? When an object has no dimensions set, the dim attribute is null. we understadn vector as 1d 
...
What happens if you define a vector with heterogeneous elements?
...
R will coerce the elements to a common type to maintain homogeneity! This is because vectors in R are meant to store only elements of the same type. For example: 
...
Ask: how to check the dimension of this the dataframe d in this list?
dim(lst$d)
dim(lst[[5]])
...
The answer is, "It depends!". Here are some guidelines to help you choose the appropriate data structure:

* Vectors and Data Frames: These are the most commonly used data structures, both in this course and beyond.

* Matrices: Primarily used for matrix algebra operations.

* Lists: Although data frames are generally preferred, lists can be useful when you need flexibility and need to store different data structures.
...
Now that we've seen what each of these data structures looks like, let's delve deeper into their characteristics!
We review all of them but focus more on vectors and lists. 

<!-- NOTES FOR NEXT TIME:  revrite last sentence to "We focus only on vectors and lists" --> 
...
<!-- 
NEXT TIME: get rid of: "(a list is still a vector in R but not an atomic one)"
-->
...
**Takeaway: All values in an atomic vector must to be of the same type**.
...
# use base R to extract only the column of interest: gives back a vector
...
Another important thing to remember about logical vectors is that they can take two values: TRUE or FALSE. 
And when you do operations with them TRUE is evaluated as 1 and FALSE is evaluated as 0
...
1. **store heterogeneous elements** (vs. all values in an atomic vector must be of the same type)
...
<!-- NOTES FOR NEXT TIME: remove this part including the visual, and go straight to in-class practice. Rewrite the slide like that:

Lists have a more complex structure than vectors, thus subsetting them also requires more attention.

Let's go straight to the in-class practice exercises to review this!
-->
...
A data frame can be seen as a list where each element (column) of the list is a vector.
...
```{r}
class(penguins)  # df
typeof(penguins) #list
```

```{r}
# extract a vector from penguins
year_v <- penguins$year
class(year_v)
```

```{r}
# do the same using the tidyverse syntax
year_v <- penguins %>% pull(year)
```
