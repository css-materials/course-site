...
Admin: 
* HW2 feedback
* HW3 check-in (see Ed Discussion for Q&A)
* Mid-course survey
* Tomorrow (Tuesday 6/25): review day
...
Two main reasons (quoted from the book):
...
**OK, you loded into R untidy data, here’s the recipe to tidy them up...** 
Identify the observations and the variables, then:
...
1. Variable values are in columns (as column names) 
1. Variable names are in rows (as cases)
...
`tidyr` documentation (every function behave differently, check the documentation to learn more!): https://tidyr.tidyverse.org/
...
This is a longitudinal dataset with four variables: country, year, number of tubercolosis (TB) cases, and population.

We demonstrate several ways to organize the same dataset. And we illustrate how to use pivoting (longer and wider), separate, and unite functions to tidy each of these datasets!
