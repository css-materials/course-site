...
To load data into R we need to use importing functions. 
...
test <- read_csv("/home/nardin/lecture/testdata.csv")

# trick
test <- read_csv(file = file.choose())
```

We first need to make sure the file is located the given path. Let's see how...
...
1. Look at your current directory by typing `getwd()` in the console
...
- Load the data and leave the `na` argument set as default. Check your loaded data and notice what happened to the missing values

- Load the data again, this time modify the `na` argument as follows:
...
- Modify the `na` argument again as follows and check again:
...
The `readr` package incluces functions to load alternative file formats that you might encounter:

* Comma separated csv `read_csv()`
* Semi column separated csv `read_csv2()`
* Tab separated files `read_tsv()`
* RDS `readRDS()` or `read_rds()`
* Excel `read_excel()`
* SPSS/Stata `haven` package
...
From the `readxl` library, part of `tidyverse` https://readxl.tidyverse.org/
Use for files you get from sources when you do not have a csv option
...
Can specify the specific worksheet by name or position
...
within the tidyverse (ggplotm dplyr etc) get them in a tidy format first, 
...
Why is this dataset messy/untidy? 
