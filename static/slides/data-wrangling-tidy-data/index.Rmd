...
From the `readxl` package https://readxl.tidyverse.org/, use `read_excel()` to load excel files:
...
We illustrate these tasks with datasets from the readings (Chapter 12 R for Data Science). Each dataset shows the same values of four variables country, year, population, and cases, but each dataset organises the values in a different way.
