...
When we talk about a bug mostly we refer to a syntactic error or a typo, but a bug might has to do with understanding R rules, data types, structures, and how to use packages. Computers are powerful tools but you need to follow their rules
...
And breaking down your problem into multiple steps to figure out exactly where the error occurred.
...
* Notation and naming guide (file names, object names...)
...
Taken from a speech given by Ronald Reagan in 1987, after the spacial Challenger exploded on take off. Reagan's address: https://youtu.be/Qa7icmqgsow
...
```r
# Optimal (short and use snake case)
day_one
day_1
...
Indentation makes the code more readable. For example, it is helpful to identify which values are part of which function. For example:
...
If you are using functions that you have not written (e.g., from packages), we do not have the ability to rename them. Sometimes functions have the same name across different packages:
...
For ex., here we are plotting with `geom_point()` and `geom_smooth()`, which automatically decides which smoothing algorithm to use to plot (default is `gem` based on your sample size):
...
Difference between creating a massage with `message()` and printing text to the console with `print()`

You can write a message with `message()` and suppress it, if you want to, with `suppressMessages()`:
...
If you write a message with `print()` there is no way to suppress it:
```
...
For example, here we have several functions that call other functions (e.g., `f` is a function that takes an input `a` and passes it into another function `g`; `g` takes an input `b` and passes it into function `h`, etc.). 
...
Use `traceback()` (often shown automatically) and read it from bottom to top, the line at the top is where the error occurred:
