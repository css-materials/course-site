...
The previous slides summarized the logic behind factor varialbes. 

In practice, you can use the package `forcats` to make your life easier when you work with categorical variables. It provides several functions, such as:
...
Run the code in R: it works, but the plot is not exactly correct... why? Look at the order of the bars and their labels.

**Challenge:** use `fct_relevel()` (from `forcats`) or `factor()` (from base R) to sort things by hand
...
#### you can do the same with `factor` (from base R)
...
<!--
Keep this trick in mind for the homework assignments (e.g., how to change a variable into a factor and manually pass labels for your graphs!)

With factors: levels and labels must match!! 

Conside putting this challange in the practice exercises for today as well
-->

--

*Takeaway: the code might run, but the output conceptually may not be what we want! Check the documentation and your data to find the best function to use to achieve your desired results*
