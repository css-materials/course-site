...
1. Search for answers in the data by transforming, visualizing, and running initial models on the data
...
- *Covariation* that how values of two variables covary (bivariate analysis)

--

> Visualizations are employed in both Exploratory and Confirmatory Data Analysis, but their use is different. In Exploratory Analysis you might generate 100 or even 1000 graphs, but not all of them will be useful for your research. In Confirmatory Analysis, you generate only a few graphs and each graph is well refined.
...
Box plot: looks at the relationship between a **continuous variable** (here cost) and a  **categorical variable** (here type). It summarizes the distribution for the continuous variable across each of the categorical variables.
...
no clear allignment on diagonal, net costs tend to be lower than adv costs for several schools, especially as the adv costs increase; in most university the average student pay less than the adv costs. It is a 2d plot bcs you are mapping two variables: one of the y and one of the x.

Link to next slide: for histogram does not make sense to map a second variable on the y, beside count/frequency. bcs you see the total distribution (vs box plot in which you see summary stats of the distribution). 
...
Multiple windows plot (histogram with facet): 
...
### TO DO: Review practice on website, change path to download, make sure everything works
...
TO DO: Add a slide or two on factors 
