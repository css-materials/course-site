...
## For other types of univariate and bivariate graphs
...
Additional info: rather than using facets to sort each distribution, we could use the color aesthetic to automatically have incorporate the `type` info into the same visualization.

We can add a fourth variable such as `depth` and render it through the size aesthetic. However, does adding `depth` make the graph more informative? 
...
### TO DO: 
+ Review practice on website, change path to download, make sure everything works, see video min 26
+ Review Assignment 2 end of video
