...
* approach 2: if no R wrapper exists, we can **directly use the API** provided by the website. In this case, the user relies on R to interact directly with the website's API.

NB: sometimes both approaches are available; other times only approach 2 is available (this is the case if no one has developed an R package to interact with that specific API)
