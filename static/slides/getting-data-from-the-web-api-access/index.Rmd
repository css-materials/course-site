...
## We also learned there are two main ways to scrape data
...
**SelectorGadget:** https://rvest.tidyverse.org/articles/selectorgadget.html (see also the previous lecture class materials for more on this!)
...
### Using an API with a wrapper: Wordbank database API example
...
### Using APIs: authentication and register for access
...
### Using an API with a wrapper and with authentication: GeoNames example
...
*Let's check today's in-class materials to learn how store username and other private info in R for this API (the process applies for all APIs)!*

<!-- we are going to register to this API and show how to store our credientials in R -->

---

### Using an API with a wrapper: other examples
...
### Using an API withour a wrapper: OMDb Movies example

**OMDb** stores info about movies, and currenty does provide a very good API but not an R wrapper
