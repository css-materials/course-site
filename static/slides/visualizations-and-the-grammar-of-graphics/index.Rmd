...
* In-class `ggplot2` practice: Gapminder data
...
But what happens if we visualize of each of these 13 datasets?
...
General approach with ALL packages in R:

You first **install** a package. You do this *only once per machine*. Everything is already installed for us in R Workbench, so you can skip this step:
```
...
Before continuing with the grammar of graphics, let's spend some time on coding style.

Below is the same code from the previous slide. Look at it, and this time focus on its style. What do you notice? 
...
* follow naming conventions (descriptive, meaningful, not too long or short, use snake_case)
...
* comments: use them parsimonously 

---

## Back to the Grammar of Graphic

Now back to the Grammar of Graphics! 

We have seen what it is, the code template, and brief overview of its components. Now we review some of these components more in detail...
